...
- **PDF の表抽出は opt-in**: GUI チェックボックス「PDFの表をMarkdown表として抽出」が ON のとき、`Converter.convert_file(extract_tables=True)` で `pdfplumber` を **追加で**起動して `find_tables` → `_format_markdown_table` で Markdown 表を出力する。pypdfium2 のテキスト抽出は維持しつつ、表 bbox 内の文字は `page.filter` で本文側から除外し重複を避ける。pdfplumber は過去に大型 PDF でハングした実績があるため (旧 311 ページ事例)、ページ単位で `try/except` し失敗時は pypdfium2 のテキスト抽出にフォールバックする。デフォルト OFF なので既存ユーザーへの影響なし。設定は `~/.mdconverter.json` の `extract_tables` キーに永続化。
