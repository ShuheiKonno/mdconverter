...
- **GUI フッターにコピーライト表記を追加** — メインウィンドウ最下段に
  `Copyright (c) 2026 Shuhei Konno` を表示。既存のバージョン表示は同一
  フッター内の右側に再配置し、左にコピーライトを並べる 2 カラム
  レイアウトに変更

### v0.4.1 (2026-04-26)

- **Excel/CSV 空セル(NaN)・連続空行の除去オプション**（opt-in、デフォルト
  OFF）— pandas が空セルを `|NaN|` として書き出す問題を後処理で吸収。
  チェックボックス ON で `|NaN|` セルを空セル化し、連続する空行を 1 行に
  圧縮して AI 消費者向けのトークン消費を削減する。第 1 列が整数（行番号
  列）のみで他列が空の行も「空行」と見なして圧縮。文字列 "NaN" を保持
  したいケースを考慮し既定 OFF。設定は `~/.mdconverter.json` の
  `compress_table_blanks` キーに永続化
- **ログ表示を別ウィンドウ化** — メインウィンドウ下部のログ欄を撤去し、
  「ログ表示」ボタンから別ウィンドウで開く方式に変更。本体ウィンドウが
  コンパクトになり、ログは内部バッファに常時蓄積されるためウィンドウを
  開けば過去のメッセージも参照可能
